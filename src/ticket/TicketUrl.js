...
import {MAIN_URL,URL_DEBUG,URL_RELEASE} from './Ticketmain';
...
export default class TicketUrl extends Component{
  ...
  constructor(props) {
    ...
    if(Api.imageUrl.startsWith("http://218.5.80.17")){
     ...
     let state = {canBack:false,renderBack:false,uri:uri+Account.user.hash+"#/billHistory",title:'电子发票'};
     this.state=state;
     this.url = state.url;
  ...
  _onNavigationStateChange=(navState)=>{
    ...
    // if (!navState.loading) {
      ...
      this.setState({ canBack: navState.canGoBack,renderBack:navState.canGoBack});
    // }
