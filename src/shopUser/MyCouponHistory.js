...
export default class MyCouponHistory extends Component{

	constructor(props) {
		super(props);
		this.state={data:{canUse:false}};
	}
	componentWillMount() {
	...
	_renderList=(rowData)=>{
		return <TouchableOpacity  onPress={()=>{this._couponDetail(rowData)}}>
				{rowData.state===2?
					<Image style={styles.list} source={require('./images/alluser.png')}>
						{this._renderTitle(rowData)}
						{this._renderContent(rowData)}
					</Image>:		<Image style={styles.list} source={require('./images/count_re.png')}>
								{this._renderTitle(rowData)}
								{this._renderContent(rowData)}
							</Image>
				}
