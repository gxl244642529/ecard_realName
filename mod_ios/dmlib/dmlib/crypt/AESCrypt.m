...
const NSInteger Nb = 4;    // number of columns in the state & expanded key
...
const Byte InvSbox[] = { // inverse s-box
...
const Byte Xtime2Sbox[] = { // combined Xtimes2[Sbox[]]
...
const Byte Xtime3Sbox[] = { // combined Xtimes3[Sbox[]]
...
const Byte Rcon[] = {0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

@interface AESCrypt()
{
    NSUInteger _keyLength;
    Byte* _key;
    NSUInteger _Nr;
    Byte _state[16];
}
...
-(id)initWithKey:(NSData*)key{
    if(self = [super init]){
        NSUInteger Nk = key.length / 4;
        _Nr = Nk + 6;
        NSUInteger len = Nb * (_Nr + 1) - Nk;
        _key = malloc(len*4 + key.length);
         _keyLength = key.length;
        
        [key getBytes:_key length:key.length];
        [self expaneKey];
    }
    return self;
}
...
-(void)encrypt:(Byte*)block index:(NSInteger)index{
    //block拷贝到_state
    memcpy(_state,block,Nb*4);
    [self addRoundKey:_key offset:0];
    for (NSUInteger round = 1; round < _Nr + 1; round++ ) {
        if (round < _Nr) {
            [self mixSubColumns];
        } else {
            [self shiftRows];
        }
        [self addRoundKey:_key offset: round * Nb * 4];
    }
    memcpy(block,_state,Nb*4);
}

-(void)decrypt:(Byte*)block index:(NSInteger)index{
    //block拷贝到_state
    memcpy(_state,block,Nb*4);
    [self addRoundKey:_key offset:_Nr*Nb*4];
    [self invShiftRows];
    for (NSUInteger round = _Nr; round--; ) {
        [self addRoundKey:_key offset: round * Nb * 4];
        if(round>0){
...
#pragma private
-(void)expaneKey{
    NSUInteger tmp0=0, tmp1=0, tmp2=0, tmp3=0, tmp4=0;
    NSUInteger idx;
    NSUInteger Nk = _keyLength/4;
    _Nr = Nk+6;
    Byte* key = _key;
    for( idx = Nk; idx < Nb * (_Nr + 1); idx++ ) {
        tmp0 = key[4*idx - 4];
        tmp1 = key[4*idx - 3];
        tmp2 = key[4*idx - 2];
        tmp3 = key[4*idx - 1];
...
-(void)addRoundKey:(Byte*)key offset:(NSUInteger)offset{
    
    for(NSUInteger idx = 0; idx < 16; ++idx){
        _state[idx] ^= key[idx + offset];
    }
