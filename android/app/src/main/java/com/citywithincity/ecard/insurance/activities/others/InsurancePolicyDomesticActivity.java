...
import com.citywithincity.ecard.insurance.models.vos.InsurancePolicyVo.Insurant;
...
@Observer
@EventHandler
public class InsurancePolicyDomesticActivity extends BaseActivity implements IOnTextClickListener {

	private InsurancePolicyVo data;
	
	@Override
	protected void onSetContent(Bundle savedInstanceState) {
		setContentView(R.layout.activity_insurance_policy_domestic);
		setTitle("保单详情");
	...
	private void initView() {
		...
		ViewUtil.setTextFieldValue(this, R.id.address, data.applicants.BUY_HOME_ADDR);
		...
		if (data.insurants != null && data.insurants.size() > 0) {
			...
			ListDataProvider<Insurant> dataProvider = new ListDataProvider<InsurancePolicyVo.Insurant>(this, R.layout.item_view_insurance_policy_domestic, new IListDataProviderListener<Insurant>() {
				...
				public void onInitializeView(View view, Insurant data,
						int position) {
					ViewUtil.setTextFieldValue(view, R.id.insured1, data.insurant_name);
					ViewUtil.setTextFieldValue(view, R.id.insured_id1, data.insurant_pid);
			...
			dataProvider.setData(data.insurants, false);
		}
